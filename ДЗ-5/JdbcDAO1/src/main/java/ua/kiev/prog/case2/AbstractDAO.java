...
public abstract class AbstractDAO<T> {
    ...
    public List<T> getAll(Class<T> cls,String... columnForShow) {
        List<T> res = new ArrayList<>();
        try {
            try (Statement st = conn.createStatement()) {
                String selectColumn = String.join(",", columnForShow);

                try (ResultSet rs = st.executeQuery("SELECT+selectColumn+ FROM " + table)) {
                    ResultSetMetaData md = rs.getMetaData();
