...
public class UrlController {
    ...
    @PostMapping("short")
    public List<UrlResultDTO> shorten(@RequestBody UrlsListDTO urlsListDTO) {
        List<UrlResultDTO> results = new ArrayList<>();
        for (String url : urlsListDTO.getUrls()){
            UrlDTO urlDTO = new UrlDTO();
            urlDTO.setUrl(url);
            long id = urlService.saveUrl(urlDTO);

            UrlResultDTO urlResultDTO = new UrlResultDTO();
            urlResultDTO.setUrl(urlDTO.getUrl());
            urlResultDTO.setShortUrl(Long.toString(id));
            results.add(urlResultDTO);
        }
        return results;
    }
}